...
class AccountGeneralLedgerReport(models.AbstractModel):
    _name = "mis.account.general.ledger"
    _description = "General Ledger Report (Custom by MIS)"
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        # print(">>> options...: " + str(options))
        # print(">>> expanded_account...: " + str(expanded_account))
        # print(">>> accounts_results...: " + str(accounts_results))
        # print(">>> taxes_results...: " + str(taxes_results))
    ...
    def _get_options_domain(self, options):
        ...
        account_code = ('4100.001.01', '4100.001.02', '4100.001.03', '4100.001.04',
                       '4100.002.01', '4100.002.02', '4100.002.03',
                       '4100.003.01', '4100.003.02', '4100.003.03', '4100.003.04', '4100.003.05',
                       '4100.004.01', '4100.004.02', '4100.004.03', '4100.004.04',
                       '4100.005.01',
                       '4100.006.01', '4100.006.02',
                       '1300.007.03', '1300.007.04', '1300.007.05', '1300.07.06',
                       '1300.007.01','1300.008.01', '2300.001',
                       '2300.001','2300.002','2300.004','2300.005','2300.007','2300.008')
        domain += [
    ...
    def _get_query_amls(self, options, expanded_account, offset=None, limit=None):
        ...
        query = '''
            SELECT
                ...
                ( CASE WHEN etax_invoice_merge.name IS NOT NULL THEN etax_invoice_merge.name
                       WHEN account_move_line__move_id.ref IS NOT NULL THEN account_move_line__move_id.ref
                  ELSE '' END ) AS invoice_commercial,
                ( CASE WHEN etax_invoice.name IS NOT NULL THEN etax_invoice.name
                     WHEN account_move_line__move_id.e_tax_vendor_bill IS NOT NULL THEN account_move_line__move_id.e_tax_vendor_bill
                  ELSE '' END ) AS etax_no
            ...
            LEFT JOIN account_move_etax_invoice_merge_rel ON account_move_etax_invoice_merge_rel.account_move_id = account_move_line__move_id.id
            LEFT JOIN etax_invoice_merge ON etax_invoice_merge.id = account_move_etax_invoice_merge_rel.etax_invoice_merge_id
            LEFT JOIN etax_invoice ON etax_invoice.id = etax_invoice_merge.e_tax_invoice_id
            WHERE %s
    ...
    def _do_query(self, options_list, expanded_account=None, fetch_lines=True):
        ...
        print(">>> Option List : " + str(options_list))
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        return {
            ...
            'columns': [
                ...
                {'name': aml['invoice_commercial'], 'title': title},
                {'name': aml['etax_no'], 'title': title},
