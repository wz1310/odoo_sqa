...
class AccountReport(models.AbstractModel):
    ...
    def _sort_lines(self, lines, options):
        ...
        for line in sorted(tree[None], key=lambda k: ('total' in k.get('class', ''), selected_sign * k['columns'][selected_column - k.get('colspan', 1)])):
            merge_tree(line)
    ...
    def open_journal_items(self, options, params):
        ...
        if options:
            ...
            #     analytic_group_ids = [int(r) for r in options['analytic_account_group']]
            #     domain = expression.AND([domain, [('analytic_account_id.group_id', 'in', analytic_group_ids)]])
            if options.get('date'):
    ...
    def get_report_informations(self, options):
        ...
        # if options.get('analytic_account_group') is not None:
        #     options['selected_analytic_account_group_names'] = [self.env['account.analytic.group'].browse(int(accg)).name for accg in options['analytic_account_group']]
        if options.get('analytic_tags') is not None:
            options['selected_analytic_tag_names'] = [self.env['account.group'].browse(int(tags)).name for tags in options['analytic_tags']]
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        # if self.env.context.get('analytic_account_group_ids'):
        #     rcontext['options']['analytic_account_group_ids'] = [
        #         {'id': accg.id, 'name': accg.name} for accg in self.env.context['analytic_account_group_ids']
        #     ]
