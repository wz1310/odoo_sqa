...
class bec_voucher_payment(models.Model):
    ...
    def amount_to_words(self):
        if self.env.user.company_id.text_amount_language_currency:
            self.text_amount = num2words(self.amount_to_text, to='currency',
                                         lang=self.env.user.company_id.text_amount_language_currency)
    
    @api.depends('line_ids.residual')
    ...
    def get_entries(self,id_d):
        ...
        self.env.cr.execute("""
            select aml.credit as credit, aa.code as code, aa.name as name, am.name as move_name,am.note 
            from voucher_payment vp 
            LEFT JOIN voucher_payment_line vpl ON vpl.payment_id = vp. ID
            LEFT JOIN account_move am ON am. ID = vpl. NAME
            LEFT JOIN account_move_line aml ON am. ID = aml.move_id
            LEFT JOIN account_account aa ON aa. ID = aml.account_id 
            where vp.id = %s and aml.credit > 0
            and aml.account_internal_type = 'payable'
    ...
    name = fields.Char('No Voucher Payment', required=True, index=True, copy=False, default='New', track_visibility='onchange')
    ...
    text_amount = fields.Char(string="Text Amount", required=False, compute="amount_to_words" )
    ...
    vendor_bank = fields.Many2one('res.partner.bank',string='Bank Partner',readonly=True, states={'draft': [('readonly', False)]})
    vendor_bank_type = fields.Many2one('res.bank',string='Bank Type',readonly=True, related="vendor_bank.bank_id",store=True)

    # @api.depends('partner_id')
    # def cek_vendor_bank(self):
    #     print('tes')
    #     for vb in self:
    #         vnd_bank = self.env['res.partner.bank'].search([('partner_id', '=', vb.partner_id.id)])
    #         return vnd_bank
    ...
    #     voucher_payment = super(bec_voucher_payment, self).write(vals)    
...
class bec_voucher_payment_line(models.Model):
    ...
    partner_id_order = fields.Many2one('res.partner', string='Vendor', domain="[('company_id', '=', False)]",tracking=True, required=True)
    name = fields.Many2one('account.move', string='Payment',
        ondelete='cascade', index=True, required=True,domain="[('type','=','in_invoice'),('state','=','posted'),('invoice_payment_state','=','not_paid')]")
    ...
    residual = fields.Monetary(string='Amount Due',
        store=True, related="name.amount_residual")
    currency_id = fields.Many2one('res.currency', string='Currency',
        required=True, readonly=True,related="name.currency_id")
    note = fields.Char('Note', readonly=True, related="name.note")
    state = fields.Selection([
            ('draft','Draft'),
            ('posted', 'Posted'),
            ('cancel', 'Cancelled'),
    ...
    invoice_payment_state = fields.Selection([
            ...
            ('paid', 'Paid'),
        ], string='Payment', index=True, readonly=True, related="name.invoice_payment_state")
    ...
    def unlink(self):
        for record in self:
            move = self.env['account.move'].search([('id','=',record.name.id)])
            move.update({
                'voucher_id' : False,
...
class inherit_register_payments(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        invoice = self.env['account.move'].search([('id','=',self._context['active_ids'][0])])
        if invoice:
...
class inherit_bec_invoice_AccountInvoice(models.Model):
    ...
    voucher_id = fields.Many2one('voucher.payment', string='Voucher Number', readonly=True)
...
class account_voucher_register(models.TransientModel):
    ...
    comment = fields.Text(string='Information')
    
    def create_request(self):
        bill = self.env['account.move'].search([('id','in',self._context['active_ids'])])
        type_inv = [] 
        state = []
        for bill in bill:
            if 'posted' not in bill.state or 'not_paid' not in bill.invoice_payment_state:
        ...
        for vendor_id in self._context['active_ids']:
            invoice = self.env['account.move'].search([('id','=',vendor_id)])
...
class bec_account_payment(models.Model):
    ...
    def post(self):
        voucher = self.env['account.move'].search([('id','=',self._context.get('active_id'))])

        # print(voucher,' mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm')
        # print(self._context.get('active_model'),' active_model')
        # exit()

        # print(voucher.type,' kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')

        #jika bukan register payment dari expense
        if self._context.get('active_model') != 'hr.expense.sheet':
            if voucher.type == 'in_invoice':
                # print(voucher.voucher_id.state,' ==============================================')
        ...
        rec = super(bec_account_payment, self).post()
