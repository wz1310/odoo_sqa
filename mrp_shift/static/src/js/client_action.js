odoo.define('mrp_shift.ClientAction', function (require) {
'use strict';
...
var clientaction = clientaction.include({
    events: {
        'click .manufacturing_period': '_onClickEditManufacturing_Period',
        'change .manufacturing_period_to_display': '_onClickEditManufacturing_Period_To_Display',
    ...
    _onClickEditManufacturing_Period_To_Display: function(){
        var value = $('.manufacturing_period_to_display').val()
        this._rpc({
            model: 'res.company',
            method: 'write',
            args: [this.companyId, {manufacturing_period_to_display: value}],
        }).then(function () {
            window.location.reload(false)
        });
    },
    _onClickEditManufacturing_Period: function (ev) {
        var value = ev.target.dataset.value
        this._rpc({
            model: 'res.company',
            method: 'write',
            args: [this.companyId, {manufacturing_period: value}],
        }).then(function () {
            window.location.reload(false)
        });
    },
...
var ClientActionUser = AbstractAction.extend({
    contentTemplate: 'mrp_shift',
    hasControlPanel: true,
    loadControlPanel: true,
    withSearchBar: true,
    events: {
        'change .o_mrp_shift_input_forcast_qty': '_onChangeForecast',
        'change .o_mrp_shift_input_replenish_qty': '_onChangeToReplenish',
        'click .o_mrp_shift_automatic_mode': '_onClickAutomaticMode',
        'click .o_mrp_shift_create': '_onClickCreate',
        'click .o_mrp_shift_edit': '_onClickEdit',
        'click .manufacturing_period': '_onClickEditManufacturing_Period',
        'change .manufacturing_period_to_display': '_onClickEditManufacturing_Period_To_Display',
        'click .o_mrp_shift_open_details': '_onClickOpenDetails',
        'click .o_mrp_shift_procurement': '_onClickReplenish',
        'click .o_mrp_shift_record_url': '_onClickRecordLink',
        'click .o_mrp_shift_unlink': '_onClickUnlink',
        'focus .o_mrp_shift_input_forcast_qty': '_onFocusForecast',
        'focus .o_mrp_shift_input_replenish_qty': '_onFocusToReplenish',
        'mouseover .o_mrp_shift_procurement': '_onMouseOverReplenish',
        'mouseout .o_mrp_shift_procurement': '_onMouseOutReplenish',
    },
    custom_events: {
        search: '_onSearch',
    },
    ...
    update_cp: function () {
        var self = this;
        this.$buttons = $(QWeb.render('mrp_shift_control_panel_buttons'));
        this._update_cp_buttons();
        var $replenishButton = this.$buttons.find('.o_mrp_shift_replenish');
        ...
        this.$buttons.find('.o_mrp_shift_create').on('click', self._onClickCreate.bind(self));
        this.$searchview_buttons = $(QWeb.render('mrp_shift_control_panel_option_buttons', {groups: self.groups}));
    ...
    _reloadContent: function () {
        ...
        return this._getState().then(function () {
            var $content = $(QWeb.render('mrp_shift', {
                widget: {
            ...
            $('.o_mrp_shift').replaceWith($content);
    ...
    _renderState: function (states) {
        for (var i = 0; i < states.length; i++) {
            ...
            var $table = $(QWeb.render('mrp_shift_production_schedule_user', {
    ...
    _saveForecast: function (productionScheduleId, dateIndex, forecastQty) {
        var self = this;
        if (!productionScheduleId || !dateIndex){
            return
        }
        this.mutex.exec(function () {
            return self._rpc({
    ...
    _update_cp_buttons: function () {
        ...
        var $addProductButton = this.$buttons.find('.o_mrp_shift_create');
        if (recodsLen) {
    ...
    _onMouseOverReplenish: function (ev) {
        ...
        var replenishClass = '.o_mrp_shift_forced_replenish';
        if (! table.length) {
            table = $('tr');
            replenishClass = '.o_mrp_shift_to_replenish';
        }
        table.find(replenishClass).addClass('o_mrp_shift_hover');
    },
    ...
    _onMouseOutReplenish: function (ev) {
        ...
        table.find('.o_mrp_shift_hover').removeClass('o_mrp_shift_hover');
    },
...
core.action_registry.add('mrp_mps_client_action_user', ClientActionUser);

return ClientActionUser;
