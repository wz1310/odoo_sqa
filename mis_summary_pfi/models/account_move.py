                ...
                for product in rec.invoice_line_ids.mapped('product_id'):
                    ...
                    for rec_tax in taxes:
                        ...
                        if tax:
                            ...
                            if self.env.company.id == 2:
                                print("account_tax.price_include",account_tax.price_include)
                    ...
                    if qty > 0:
                        ...
                        y_app = []
                        for y in rec.summary_invoice_line_ids:
                            print("PROD ID",y.product_id.id)
                            print("HIDE DISS",[x.quantity if 'discount' not in y.name else 0])
                            print("HIDE DISS",sum([x.quantity if 'Free Product' in x.name else 0 for x in self.invoice_line_ids.filtered(
                                lambda d:d.product_id.id == x.product_id.id)]))
                            print("PRODPROD DISS",[x.product_id if 'Free Product' in x.name else 0 for x in self.invoice_line_ids.filtered(
                                lambda d:d.product_id.id == x.product_id.id)])
                            print("PRODPROD DISS",[x.name if 'Free Product' in x.name else x.name for x in self.invoice_line_ids.filtered(
                                lambda d:d.product_id.id == x.product_id.id)])
                            print("DISC FIX AMOUNT", y.product_id.id)
                            print("REC PICKING IDS", [x.disc_fix_amount for x in rec.picking_ids.sale_id.order_line.filtered(
                                lambda d:d.promotion_product_id == str(y.product_id.id))])
