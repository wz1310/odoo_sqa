...
class WizardARReport(models.TransientModel):
	_name = 'wizard.ar.report'
	_description = 'Wizard AR Report'
	...
	def _get_data_purchase(self):
		...
		if self.partner_ids:
			if len(self.partner_ids) == 1:
				data_tuple = "(%s)" % self.partner_ids.id
			else:
				data_tuple = tuple(self.partner_ids.ids)
