...
class GlobalDivisionSummarySalesReport(models.Model):
    _name = "report.global.division.summary.sales"
    ...
    def get_main_request(self):
        request = """
            ...
            SELECT 
                CONCAT(sr.team_id, sr.product_id) as id,
                sr.team_id,
                sr.product_id,
                ...
                (SELECT sum(tl.target_per_day) from sales_user_target_line tl
                    ...
                    AND tl.product_id = sr.product_id AND tl.team_id = sr.team_id
                    group by tl.product_id) as qty_target,
                coalesce((sum(sr.qty_delivered) - (SELECT sum(tl.target_per_day) from sales_user_target_line tl
                    WHERE tl.year = EXTRACT(YEAR from sr.order_date)::TEXT AND tl.month = EXTRACT(MONTH from sr.order_date)::TEXT
                    AND tl.product_id = sr.product_id AND tl.team_id = sr.team_id
                    group by tl.product_id) ), 0) as accumulate,
                coalesce((sum(sr.qty_delivered)/ (SELECT sum(tl.target_per_day) from sales_user_target_line tl
                    WHERE tl.year = EXTRACT(YEAR from sr.order_date)::TEXT AND tl.month = EXTRACT(MONTH from sr.order_date)::TEXT
                    AND tl.product_id = sr.product_id AND tl.team_id = sr.team_id
                    group by tl.product_id)), 0) * 100 as persentase,
                sr.company_id
            FROM sale_report sr
			GROUP BY sr.team_id,sr.product_id,sr.date::DATE,sr.order_date,sr.company_id,CONCAT(sr.team_id, sr.product_id)
                """ % (self._table)
        return request
