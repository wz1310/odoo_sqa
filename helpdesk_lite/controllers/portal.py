...
#             'stage': {'input': 'stage', 'label': _('Search in Stages')},
    ...
    def my_tickets_ticket(self, ticket_id=None, **kw):
        ...
        modelz = 'helpdesk_lite.ticket'
        attach = request.env['ir.attachment'].sudo().search([('res_model','=',modelz),('res_id','=',ticket_id)])
        return request.render("helpdesk_lite.my_tickets_ticket", {'ticket': ticket,'attach':attach})
    ...
    def ticket_new(self, **kw):
        ...
        ctg = request.env['helpdesk_lite.sub.category'].sudo().search([],order="id asc")
        pri_default = '1'
    ...
    @http.route('/website_form/', csrf=False, type="http", methods=['POST', 'GET'], auth="public", website=True)
    def search_contents(self, **kw):
        """get search result for auto suggestions"""
        try:
            strings = '%' + kw.get('name') + '%'
            print("NMAMAMAMAMA",kw.get('name'))
            job = request.env['helpdesk_lite.ticket'].with_user(SUPERUSER_ID).search([])
            sql = """select name as name, name as value from helpdesk_lite_subject where name ILIKE '{}' GROUP BY name"""
