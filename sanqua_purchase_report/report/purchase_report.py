...
class PurchaseReceiptReport(models.Model):
    ...
    # qty_backorder = fields.Float(string='Qty Backorder')
    ...
    def init(self):
        ...
        self.env.cr.execute("""CREATE or REPLACE VIEW %s as (
            %s
            FROM ( %s )
            WHERE sp.state not in ('cancel') AND spt.code = 'incoming'
            %s
    ...
    def _select(self):
        select_str = """
            WITH currency_rate as (%s)
                SELECT
                    po.id as order_id,
                    CONCAT(lpad(po.id::TEXT, 5,'0'),rpad(min(l.id)::TEXT,5,'0'),rpad(sp.id::TEXT,5,'0'))::BIGINT as id,
                    ...
                    sum(sm.product_uom_qty * l.price_unit / COALESCE(po.currency_rate, 1.0))::decimal(16,2) as untaxed_total,
                    sum(sm.product_uom_qty / line_uom.factor * product_uom.factor) as qty_ordered,
                    sum(sml.qty_done / line_uom.factor * product_uom.factor) as qty_received,
                    sum(sml.qty_done / line_uom.factor * product_uom.factor) as qty_billed,
                    sum(l.product_qty / line_uom.factor * product_uom.factor) - sum(l.qty_received / line_uom.factor * product_uom.factor) as qty_backorder,
                    case when t.purchase_method = 'purchase' 
                         then sum(l.product_qty / line_uom.factor * product_uom.factor) - sum(l.qty_invoiced / line_uom.factor * product_uom.factor)
                         else sum(l.qty_received / line_uom.factor * product_uom.factor) - sum(l.qty_invoiced / line_uom.factor * product_uom.factor)
                    end as qty_to_be_billed, 
                    sp.scheduled_date,sp.id as picking_id,sp.date_done, spt.warehouse_id as picking_type_id
    ...
    def _from(self):
        from_str = """
            purchase_order_line l
				...
				left join stock_picking sp ON sm.picking_id = sp.id AND sp.picking_type_id = po.picking_type_id
    ...
    def _group_by(self):
        group_by_str = """
            GROUP BY
                ...
                analytic_account.id,
                po.id, sp.scheduled_date,sp.id,sp.date_done, spt.warehouse_id
        """
        return group_by_str