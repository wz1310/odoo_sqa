...
class MoChangeRequest(models.Model):
    _name = 'mo.change.request'
    _description = "MO Change Request"
    _inherit = ["approval.matrix.mixin",'mail.thread', 'mail.activity.mixin']

    name = fields.Char(string='Name', required=True, copy=False, readonly=True, states={'draft': [('readonly', False)]}, index=True, default=lambda self:self.env['ir.sequence'].next_by_code('seq.mo.change.request'))

    mo_id = fields.Many2one('mrp.production', string='Manufacturing order',required=True, track_visibility='onchange')
    field_id = fields.Many2one('ir.model.fields', string='Field', domain=[('model_id','=','mrp.production'),('id','in',['14220','14221','14222','14223','15102','12221','14206','14201'])], track_visibility='onchange')
    ...
    line_ids = fields.One2many('mo.change.request.line', 'mo_change_request_id', string='Lines')
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('seq.mo.change.request')
        result = super(MoChangeRequest, self).create(vals)
        return result
    ...
    def _onchange_field_id(self):
        ...
        if self.mo_id:
            mo_id = self.env['mrp.production'].browse(self.mo_id.id)
            if self.field_id:
                if self.field_type == 'many2one':
                    old_value_m2o = getattr(self.mo_id,self.field_id.name).id
                    if old_value_m2o:
                    ...
                    self.old_value_char = mo_id[self.field_id.name]
            ...
            if self.field_type in ['many2one']:
                ...
                for rec in self.line_ids:
                    ...
                    if rec.delete == True:
                        ...
                        if rec.update_value:
                            models = models.browse(eval(rec.values).get('id'))
                            self.mo_id.sudo().message_post(body=self._get_message_update(rec.update_value,eval(rec.values).get('id')))
                ...
                this_field = self.mo_id.mapped(self.field_id.name)[0]
                new_value = self.new_value_char
                self.mo_id.write({
                    self.field_id.name : new_value
                })
                msgs.append("%s : %s → %s (Changed by Edit MO)"%(self.field_id.field_description,this_field,new_value))
                self.mo_id.message_post(body=msgs[0])
            self.state = 'approved'
    ...
    def _get_data_o2m(self):
        model = getattr(self.mo_id,self.field_id.name)
        line_ids = [(5,0,0)]
    ...
    def create_direct_form(self,vals,request_id,model_name):
        ...
        mo_change_request_id = self.env['mo.change.request'].browse(request_id)
        mo_change_request_id.line_ids = [(0,0,data)]
        model.sudo().unlink()
        return {'type': 'ir.actions.act_window_close'}

class MoChangeRequestLine(models.Model):
    _name = 'mo.change.request.line'
    _description = "MO Change Request Line"
    _inherit = ['mail.thread', 'mail.activity.mixin']

    mo_change_request_id = fields.Many2one('mo.change.request', string='MO Change Request')
    ...
    state = fields.Selection(related='mo_change_request_id.state', string='State')
    ...
    def btn_update(self):
        view = self.env.ref('mis_mo_flow.mo_change_request_line_field_view_form')
        return {
                ...
                'view_type': 'form',
                'views': [(view.id, 'form')],
                'res_model': 'mo.change.request.line.field',
                'target': 'new',
...
class MoChangeRequestLineField(models.Model):
    _name = 'mo.change.request.line.field'
    ...
    def confirm(self):
        mo_change_request_id = self.env['mo.change.request.line'].browse(self._context.get('id'))
        update_value = eval(mo_change_request_id.update_value) if mo_change_request_id.update_value else {}
        update_value[self.field_id.name] = self.new_value_m2o if self.field_type == 'many2one' else self.new_value_char
        mo_change_request_id.update_value = update_value
        mo_change_request_id.is_update = True