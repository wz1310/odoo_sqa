...
class WizardPurchaseOrderRawMaterialReport(models.TransientModel):
    _name = 'wizard.purchase.order.return.report'
    _description = 'Wizard Purchase Order Return Report'
    ...
    vendor_ids = fields.Many2many('res.partner', string="Vendor", domain=[
                                  ('supplier', '=', True)])
    product_ids = fields.Many2many('product.product', string='Products')
    group_by = fields.Selection(
        [('product', 'Product'), ('vendor', 'Vendor')], string='Group By', default='product')

    def button_print(self):
        data = self._get_data_purchase()
        if not data:
            raise ValidationError(
                _("Data Tidak ditemukan, Silahkan coba dengan Filter lainnya"))
    ...
    def _get_data_purchase(self):
        ...
        query = """
            ...
            WHERE sp.origin LIKE %s AND sp.state = 'done' AND po.company_id = %s AND sp.scheduled_date BETWEEN %s AND %s """ + vendor_filter+product_filter + """
